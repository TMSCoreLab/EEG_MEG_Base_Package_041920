...
%%  Define observation points in the plane (MsxMs observation points)
Ms = 200;
...
%in      = 1:size(pointsXY, 1);  %   plot field outside skin
...
Epri           = zeros(Ms*Ms, 3);
Esec           = zeros(Ms*Ms, 3);
[Epri(in, :), ~]= bemf3_inc_field_electric(strdipolePplus,...
                                        strdipolePminus,...
                                        strdipolesig, strdipoleCurrent, ...                                        
                                        P, t, pointsXY(in, :), Area, normals, 0, 0); 
...
%% Plot the current density in the cross-section
figure;
...
th2 = 0;            %   in A/m^2
levels      = 20;
bemf2_graphics_vol_field(temp, th1, th2, levels, x, y); hold on;
xlabel('Distance x, mm');
ylabel('Distance y, mm');
